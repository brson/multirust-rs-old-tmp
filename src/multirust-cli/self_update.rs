...
//! * multirust-setup copies bins and handlinks into place
//!
...
pub fn uninstall(no_prompt: bool) -> Result<()> {
    ...
    if multirust_dir.exists() {
        ...
        // FIXME
        //cfg.notify_handler.call(Notification::MissingFileDuringSelfUninstall(multirust_dir.into()));
    }
...
fn delete_multirust_and_cargo_home() -> Result<()> {
    ...
    use winapi::{FILE_SHARE_DELETE,
                 FILE_SHARE_READ, INVALID_HANDLE_VALUE,
                 FILE_FLAG_DELETE_ON_CLOSE, DWORD, SECURITY_ATTRIBUTES,
                 STARTUPINFOW, PROCESS_INFORMATION, OPEN_EXISTING,
                 CREATE_UNICODE_ENVIRONMENT, GENERIC_READ};
    use kernel32::{CreateFileW, CloseHandle, CreateProcessW};
    ...
    unsafe {
        ...
        // Create the gc exe process, which will inherit a handle to
        // its own file. It will wait for this process to exit then
        // delete CARGO_HOME.
        let r = CreateProcessW(gc_exe_win.as_ptr(),
                               ptr::null_mut(),
        ...
        // Sleep long enough to let the OS open the other process and increase
        // its file handle count so we don't delete it before its run
...
#[cfg(windows)]
pub fn complete_windows_install() -> Result<()> {
    use winapi::{STARTUPINFOW, PROCESS_INFORMATION, DWORD};
    ...
    let mut rm_gc_exe_win: Vec<_> = rm_gc_exe.encode_wide().collect();
    rm_gc_exe_win.push(0);
    ...
    unsafe {
        ...
        let r = CreateProcessW(ptr::null(),
                               rm_gc_exe_win.as_mut_ptr(),
                               ptr::null_mut(), ptr::null_mut(),
                               1, // inherit handles
                               0,
                               ptr::null_mut(), ptr::null(),
...
fn wait_for_parent() -> Result<()> {
    ...
    use winapi::{PROCESSENTRY32, INVALID_HANDLE_VALUE, DWORD,
    ...
    unsafe {
        ...
        let res = WaitForSingleObject(parent, 1000);
